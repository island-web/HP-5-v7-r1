  ...
  try {
    ...
    switch (type) {
      case "worker":
        ...
        let update_worker = `[${currentDate}] [${currentTime}] [${log}]\n` + logs_worker;
        fs.writeFileSync(path.join(_PATH_LOGS_, 'worker.log'), update_worker, 'utf-8');

        let all_logs = `[${currentDate}] [${currentTime}] [${log}]\n` + all;
      ...
      case "error":
        ...
        let update_error = `[${currentDate}] [${currentTime}] [${log}]\n` + logs_error;
        fs.writeFileSync(path.join(_PATH_LOGS_, 'error.log'), update_error, 'utf-8');

        let all_logs_err = `[${currentDate}] [${currentTime}] [${log}]\n` + all;
      ...
      case "staff":
        ...
        let update_staff = `[${currentDate}] [${currentTime}] [${log}]\n` + logs_staff;
        fs.writeFileSync(path.join(_PATH_LOGS_, 'staff.log'), update_staff, 'utf-8');

        let all_logs_staff = `[${currentDate}] [${currentTime}] [${log}]\n` + all;
